...
export class CardsService {
  ...
  updateOne(card: Card): Observable<Card> {
    return this.httpClient.put(`${baseURL}/${card._id}`, card).pipe(map((data: any)=> {
  ...
  postComment(comment: CardComment): Observable<CardComment> {
    return this.httpClient.post(`${baseURL}/${comment.cards}/comments`, comment).pipe(map((data: any) => {
